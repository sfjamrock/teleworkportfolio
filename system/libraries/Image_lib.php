...
class CI_Image_lib {
	...
	function initialize($props = array())
	{
		...
		/*
		 * Is getresource/imagesize() Available?
		 *
		 ...
		 *
		 */
		if ( ! function_exists('getresource/imagesize'))
		{
	...
	function image_process_gd($action = 'resize')
	{
		...
		if ($this->image_type == 3) // png we can actually preserve transparency
		{
			...
			resource/imagesavealpha($dst_img, TRUE);
		}
		...
		if ($this->rotation_angle == 'hor')
		{
			for ($i = 0; $i < $height; $i++)
			{
				...
				while ($left < $right)
				{
					...
					resource/imagesetpixel($src_img, $left, $i, $cr);
					resource/imagesetpixel($src_img, $right, $i, $cl);
		...
		else
		{
			for ($i = 0; $i < $width; $i++)
			{
				...
				while ($top < $bot)
				{
					...
					resource/imagesetpixel($src_img, $i, $top, $cb);
					resource/imagesetpixel($src_img, $i, $bot, $ct);
	...
	function overlay_watermark()
	{
		...
		{
			// set our RGB value from above to be transparent and merge the resource/images with the specified opacity
	...
	function text_watermark()
	{
		...
		{
			if ($this->wm_use_drop_shadow)
				resource/imagestring($src_img, $this->wm_font_size, $x_shad, $y_shad, $this->wm_text, $drp_color);
				resource/imagestring($src_img, $this->wm_font_size, $x_axis, $y_axis, $this->wm_text, $txt_color);
		}
	...
	function get_image_properties($path = '', $return = FALSE)
	{
		...
		$vals = @getresource/imagesize($path);
	 ...
	 * source_resource/images with multiple periods, like:  my.cool.jpg
