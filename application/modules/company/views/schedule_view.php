          ...
          <div class="table-responsive">
            <table class="table table-striped">
              <thead>
				...
				<?php foreach($user_schedule as $row): ?>
				<tr>
					<td colspan="8" style="text-align:center">
								<a href="#">
			  ...
			  <?php foreach($location_schedule as $row2): ?>
              ...
              <tbody>
                <tr>
                  <td  rowspan="2"><?php echo $row2->name; ?></td>
					...
					<!-- Clock In Timesheet Area Start-->
						...
						<td>						
							<?php foreach($schedule as $row3) : ?>
								<?php if($row2->location_id==$row3->location_id && $row2->user_id==$row3->user_id) : ?>
									<?php $starttime = new DateTime($row3->start_time);  ?>
				...
				<tr>
					<td>
						<?php foreach($schedule as $row3) : ?>
							<?php if($row2->location_id==$row3->location_id && $row2->user_id==$row3->user_id) : ?>
								<?php $endime = new DateTime($row3->end_time); ?>
					...
					<td>	<?php foreach($schedule as $row3) : ?>
							<?php if($row2->location_id==$row3->location_id && $row2->user_id==$row3->user_id) : ?>
								<?php $endime = new DateTime($row3->end_time); ?>
				...
				<tr>
					...
					<td>
						<?php foreach($schedule as $row3) : ?>
							<?php if($row2->location_id==$row3->location_id && $row2->user_id==$row3->user_id) : ?>
									<?php $starttime = new DateTime($row3->start_time); $endime = new DateTime($row3->end_time);
